...
uci set tinc.RT2.Address=203.0.113.2
...
cat /etc/tinc/LAB/ed25519_key.pub > /etc/tinc/LAB/hosts/RT2
...
uci add_list firewall.@rule[-1].dest_ip='203.0.113.2'
uci commit
...
uci add_list tinc.RT3.Address=203.0.113.3
uci set tinc.RT3.Subnet=10.3.1.0/24

# initiate a VPN connection to RT3
uci add_list tinc.LAB.ConnectTo=RT3

# get the public key from RT3 and store it in /etc/tinc/LAB/hosts/RT3
...
#!/bin/sh
ip addr add 10.2.1.2/32 dev \$INTERFACE
...
ip addr del 10.2.1.2/32 dev \$INTERFACE
EOF
